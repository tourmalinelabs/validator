...
const Validator = function (rawSchema, options = {}) {
    var testTypes = {
        any: {
            test: function () {
                return true;
        ...
        regex: {
            test: function (valueToTest, testValue) {
                var pieces = testValue.split("/"), modifiers = last(pieces), pattern;
                if (pieces.length) {
                    pieces.pop();
                }
                if (pieces.length) {
    ...
    this.test = function (rawDataToTest) {
        ...
        foreach(schema, function runTestsOnGroup(tests, name) {
            ...
            if (containsTestWithName(tests, "required") ||
