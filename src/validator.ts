  ...
  var Validator = function (rawSchema, options) {
    var testTypes = {
      ...
      minimumLength: {
        test: function (valueToTest, testValue) {
          return (
            isArrayOrString(valueToTest) && valueToTest.length >= testValue
          );
        },
        message: function (name, testValue) {
      ...
      maximumLength: {
        ...
        message: function (name, testValue) {
          return (
            name + ' cannot exceed a length of ' + testValue + ' characters'
          );
        },
      },
      regex: {
      ...
      test: function (rawDataToTest) {
        ...
        foreach(schema, function runTestsOnGroup (tests, name) {
          ...
          if (
            ...
            if (
              !(
                containsTestWithName(tests, 'allowNull') && valueToTest === null
              )
            ) {
